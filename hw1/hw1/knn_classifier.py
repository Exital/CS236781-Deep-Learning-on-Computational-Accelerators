...
from hw1.dataloaders import create_train_validation_loaders
...
class KNNClassifier(object):
    ...
    def train(self, dl_train: DataLoader):
        ...
        # ====== YOUR CODE: ======
        x, y = dataloader_utils.flatten(dl_train)
        # ========================
    ...
    def predict(self, x_test: Tensor):
        ...
        for i in range(n_test):
            ...
            # ====== YOUR CODE: ======
            res, ind = dist_matrix[i].topk(self.k, largest=False)
            labels = [self.y_train[x] for x in ind]
...
def l2_dist(x1: Tensor, x2: Tensor):
    ...
    dists = None
    # ====== YOUR CODE: ======
    n1, n2 = x1.size(0), x2.size(0)  # here both n1 and n2 have the value 2
    norm1 = torch.sum(x1 ** 2, dim=1)
    norm2 = torch.sum(x2 ** 2, dim=1)

    x1topow2 = norm1.reshape([-1, 1])
    x2topow2 = norm2.reshape([1, -1])

    norms = x1topow2 + x2topow2
    dists = torch.sqrt(norms - 2 * x1.mm(x2.t()))
    # ========================
...
def accuracy(y: Tensor, y_pred: Tensor):
    ...
    accuracy = None
    # ====== YOUR CODE: ======
    accuracy = (torch.eq(y, y_pred).sum().item()) / y.numel()
    # ========================
...
def find_best_k(ds_train: Dataset, k_choices, num_folds):
    ...
    for i, k in enumerate(k_choices):
        ...
        for m in range(num_folds):
            ratio = 1/num_folds
            tr, val = create_train_validation_loaders(ds_train, ratio)
            model.train(tr)
            x_val, y_val = dataloader_utils.flatten(val)
            pred = model.predict(x_val)
            acc = accuracy(y_val, pred)
            in_lst.append(acc)
        # ========================
        accuracies.append(in_lst)

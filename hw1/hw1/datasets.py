...
def random_labelled_image(
    ...
    image.to(dtype)
    label = int(torch.randint(low=0, high=num_classes - 1, size=(1,), dtype=torch.int))
    # ========================
    return image, label
...
class RandomImageDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, int]:
        ...
        # ====== YOUR CODE: ======
        if abs(index) not in range(0, self.num_samples):
            raise ValueError
        else:
            with torch_temporary_seed(index % self.num_samples):
                data = random_labelled_image(self.image_dim, self.num_classes)
                return data
        # ========================
...
class SubsetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return self.source_dataset[index % self.subset_len + self.offset]
